...
<div class="product-card group bg-white rounded-lg overflow-hidden border border-gray-200 
    ...
    <div class="relative overflow-hidden bg-gray-50">
        <a href="{{ route('products.show', $product->slug) }}" class="block">
            <img src="{{ $product->thumbnail }}" alt="{{ $product->name }}"
                class="w-full aspect-[3/4] object-cover object-center transform group-hover:scale-105 transition-transform duration-500 will-change-transform"
                loading="lazy">
        </a>
        ...
        @if($badge)
            <span
                class="absolute top-2.5 left-2.5 bg-{{ $badgeColor }}-500 text-white text-[10px] font-medium px-2 py-0.5 rounded shadow-sm">
                {{ $badge }}
            </span>
        @endif
    ...
    <div class="p-3 flex flex-col flex-1">
        ...
        <a href="{{ route('products.show', $product->slug) }}">
            <h3
                class="text-sm font-medium text-gray-800 mb-1.5 line-clamp-2 hover:text-primary transition-colors leading-snug">
                {{ $product->name }}
            </h3>
        </a>
        ...
        <div class="flex gap-1.5 mt-auto">
            <button
                class="add-to-cart-btn flex-1 bg-primary text-white py-1.5 rounded-lg font-normal text-sm hover:bg-blue-600 transition-colors flex items-center justify-center gap-1.5 tap-effect min-w-0"
...
@isset($oldProductCard)
    <div class="product-card bg-white rounded-2xl overflow-hidden shadow-sm border border-gray-100">
        <div class="relative">
            ...
            @if($badge)
                <span
                    class="absolute top-2 left-2 bg-{{ $badgeColor }}-500 text-white text-[10px] md:text-xs font-semibold px-2 py-1 rounded-full">
                    {{ $badge }}
                </span>
            @endif
        ...
        <div class="p-3 md:p-4">
            <a href="{{ route('products.show', $product->slug) }}">
                <h3
                    class="text-sm md:text-base font-medium text-black mb-1 line-clamp-2 hover:text-primary transition">
                    {{ $product->name }}
                </h3>
            </a>
            ...
            <button
                class="add-to-cart-btn w-full bg-primary text-white py-2 md:py-2.5 rounded-xl font-semibold text-xs md:text-sm hover:bg-blue-600 transition tap-effect"
                onclick="handleProductCardAddToCart({{ $product->id }}, {{ $product->variants->count() }})"
                data-product-id="{{ $product->id }}">
