...
<header id="mainHeader"
    class="sticky top-0 z-50 bg-white/95 backdrop-blur-lg border-b border-gray-100/50 transition-all duration-300">
    ...
    <div class="hidden md:block bg-gradient-to-r from-gray-900 to-gray-800 text-white">
        <div class="max-w-7xl mx-auto px-6 py-2 flex items-center justify-between text-xs">
            ...
            <div class="flex items-center gap-4">
                <a href="{{ route('track-order.index') }}" class="hover:text-primary transition">Track Order</a>
        ...
        <div class="flex items-center justify-between h-16 md:h-20">
            ...
            <div class="hidden md:flex flex-1 max-w-xl mx-8">
                <button onclick="openSearch()" class="relative w-full group text-left">
                    <div
                        class="w-full bg-gray-50 border border-gray-200 rounded-2xl py-3 pl-12 pr-6 text-sm text-gray-400 hover:border-gray-300 hover:bg-white transition-all duration-300">
                    ...
                    <span
                        class="absolute right-3 top-1/2 -translate-y-1/2 bg-gray-200 text-gray-500 px-2 py-1 rounded-lg text-xs font-medium">
                        Ctrl+K
                    </span>
                </button>
            </div>
            ...
            <div class="flex items-center gap-1 md:gap-2">
                ...
                {{-- Wishlist --}}
                {{--<button class="hidden md:flex icon-btn p-2.5 rounded-xl items-center gap-2 group"
                    aria-label="Wishlist">
                    <div class="relative">
                        <i class="far fa-heart text-lg text-gray-600 group-hover:text-primary transition-colors"></i>
                        <span
                            class="absolute -top-2 -right-2 bg-red-500 text-white text-[10px] w-4 h-4 rounded-full flex items-center justify-center font-bold">2</span>
                    </div>
                    <span
                        class="text-sm text-gray-600 group-hover:text-primary transition-colors hidden lg:block">Wishlist</span>
                </button>--}}
                ...
                <button class="icon-btn p-2.5 rounded-xl flex items-center gap-2 group" aria-label="Cart"
                    ...
                    <div class="relative">
                        <i
                            class="fas fa-shopping-bag text-lg text-gray-600 group-hover:text-primary transition-colors"></i>
                        <span
                            class="absolute -top-2 -right-2 bg-primary text-white text-[10px] w-4 h-4 rounded-full flex items-center justify-center font-bold floating-badge"
                    ...
                    $isAdmin = $user->role == \App\Enums\UserRole::CUSTOMER ? false : true;
                                ...
                                <div class="relative group" id="profileDropdown">
                                    <button onclick="toggleProfileDropdown(event)"
                                        class="icon-btn p-2 rounded-xl flex items-center gap-2" aria-label="Profile">
                                        <div
                                            class="w-9 h-9 rounded-xl bg-gradient-to-br from-primary to-blue-600 flex items-center justify-center text-white font-semibold text-sm shadow-lg shadow-primary/25">
                                    ...
                                    <div id="profileDropdownMenu"
                                        ...
                                        <div class="border-t border-gray-100 mt-2 pt-2">
                                            <form action="{{ route('auth.logout') }}" method="POST" id="logoutForm">
                                                @csrf
                                                <button type="button"
                                                    class="flex items-center gap-3 px-4 py-2.5 hover:bg-red-50 transition w-full text-left"
                ...
                @else
                    <button onclick="openAuthModal('login')" class="icon-btn p-2 rounded-xl flex items-center gap-2 group"
                        aria-label="Profile">
                        <div
                            class="w-9 h-9 rounded-xl bg-gradient-to-br from-primary to-blue-600 flex items-center justify-center text-white font-semibold text-sm shadow-lg shadow-primary/25">
            ...
            <div class="max-w-7xl mx-auto px-6">
                <nav class="flex items-center gap-8 h-12">
                    ...
                    <div class="relative group">
                        <button
                            class="flex items-center gap-2 text-sm font-medium text-gray-700 hover:text-primary transition-colors h-12">
                            <i class="fas fa-th-large text-primary"></i>
                            All Categories
                            <i
                                class="fas fa-chevron-down text-xs text-gray-400 group-hover:rotate-180 transition-transform duration-200"></i>
                        </button>

                        {{-- Mega Menu Dropdown --}}
                        <div
                            class="absolute top-full left-0 w-[800px] bg-white rounded-2xl shadow-2xl border border-gray-100 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 transform translate-y-2 group-hover:translate-y-0 z-50">
                            <div class="p-6">
                                ...
                                <div class="mt-6 pt-6 border-t border-gray-100">
                                    <div class="grid grid-cols-2 gap-4">
                                        <a href="{{ route('products.index') }}?collection=eid"
                                            class="relative overflow-hidden rounded-xl bg-gradient-to-r from-orange-500 to-red-500 p-5 text-white group">
                                            <div class="relative z-10">
                                                ...
                                                <span
                                                    class="inline-flex items-center gap-1 text-sm font-semibold mt-3 group-hover:gap-2 transition-all">
                                        ...
                                        <a href="{{ route('products.index') }}?collection=new"
                                            class="relative overflow-hidden rounded-xl bg-gradient-to-r from-primary to-blue-600 p-5 text-white group">
                                            <div class="relative z-10">
                        ...
                        <div class="relative group">
                            <a href="{{ route('products.index') }}?categories={{ $category->slug }}"
                                ...
                                @if($category->children->isNotEmpty())
                                    <i
                                        class="fas fa-chevron-down text-[10px] text-gray-400 group-hover:rotate-180 transition-transform duration-200"></i>
                                @endif
                            </a>
                            @if($category->children->isNotEmpty())
                                <div
                    ...
                    <a href="{{ route('products.index') }}?sale=1"
                        class="text-sm font-medium text-red-500 hover:text-red-600 transition-colors flex items-center gap-1">
