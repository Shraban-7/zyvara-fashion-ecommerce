...
<button onclick="openCartDrawer()" id="floatingCartBtn" class="fixed bottom-24 right-4 md:bottom-8 md:right-8 z-40 w-14 h-14 bg-primary text-white rounded-full shadow-lg shadow-primary/30 flex items-center justify-center hover:bg-blue-600 transition-all tap-effect group">
...
<div id="cartDrawer" class="fixed top-0 right-0 h-full w-full sm:w-[420px] bg-white z-50 shadow-2xl transform translate-x-full transition-transform duration-300 ease-out flex flex-col">
    ...
    <div class="flex items-center justify-between px-5 py-4 border-b border-gray-100">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h2 class="text-lg font-bold text-black">Shopping Cart</h2>
    ...
    <div id="emptyCartState" class="hidden flex-1 flex flex-col items-center justify-center px-5 py-10">
        ...
        <a href="{{ route('products.index') }}" onclick="closeCartDrawer()" class="bg-primary text-white px-6 py-3 rounded-xl font-semibold text-sm hover:bg-blue-600 transition tap-effect">
            Start Shopping
        </a>
    </div>
    ...
    <div class="px-5 py-3 border-t border-gray-100" id="couponSection">
        <div class="flex gap-2">
            <div class="flex-1 relative">
                <input type="text" placeholder="Enter coupon code" class="w-full h-11 px-4 pr-10 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-primary transition">
    ...
    <div class="px-5 py-4 border-t border-gray-100 bg-white" id="cartFooter">
        ...
        <div class="space-y-2 mb-4">
            ...
            <div class="flex items-center justify-between">
                ...
                <span class="text-xl font-bold text-primary" id="cartTotal">৳0</span>
            </div>
        </div>

        {{-- Checkout Button --}}
        <a href="{{ route('checkout.index') }}" class="w-full bg-primary text-white py-4 rounded-xl font-bold text-center hover:bg-blue-600 transition tap-effect shadow-lg shadow-primary/20 flex items-center justify-center gap-2 mb-3">
            <i class="fas fa-lock text-sm"></i>
            Proceed to Checkout
        </a>

        {{-- Continue Shopping --}}
        <button onclick="closeCartDrawer()" class="w-full text-center text-sm text-gray-500 hover:text-primary transition py-2">
