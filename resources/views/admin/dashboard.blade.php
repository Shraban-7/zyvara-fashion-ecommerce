...
<main class="space-y-6 p-1">
    ...
    <section class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <form method="GET">
            <div class="relative">
                ...
                <i data-lucide="calendar-days" class="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-400 pointer-events-none"></i>
    ...
    <section class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        @foreach($stats as $s)
        <article class="rounded-2xl border border-slate-100 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
    ...
    <section class="grid gap-6 xl:grid-cols-3">
        ...
        <article class="rounded-2xl border border-slate-100 bg-white p-5 shadow-sm">
            ...
            <div class="space-y-4">
                @foreach([
                    ['label'=>'Website','pct'=>$websitePercentage,'orders'=>$websiteOrders,'color'=>'bg-emerald-500','dot'=>'bg-emerald-500'],
                    ['label'=>'POS',    'pct'=>$posPercentage,    'orders'=>$posOrders,    'color'=>'bg-amber-500', 'dot'=>'bg-amber-500'],
                ] as $ch)
                <div>
                    ...
                    <div class="h-1.5 rounded-full bg-slate-100">
                        <div class="h-1.5 rounded-full {{ $ch['color'] }} transition-all duration-500" style="width:{{ $ch['pct'] }}%"></div>
                    </div>
    ...
    <section class="grid gap-6 xl:grid-cols-3">
                ...
                <table class="w-full min-w-[640px] text-left">
                    ...
                    <tbody class="divide-y divide-slate-50">
                        ...
                        <tr class="transition hover:bg-slate-50/60">
                            <td class="px-5 py-3.5 text-sm font-semibold text-slate-900">#{{ $order['order_number'] }}</td>
                            <td class="px-5 py-3.5">
                                <div class="flex items-center gap-3">
                                    <div class="grid h-8 w-8 place-items-center rounded-full bg-indigo-100 text-xs font-bold text-indigo-600">{{ $initials }}</div>
                                    <div>
                            ...
                            <td class="px-5 py-3.5 text-sm font-semibold text-slate-900">৳{{ number_format($order['total'],2) }}</td>
                            <td class="px-5 py-3.5">
                                <span class="rounded-full px-2.5 py-1 text-xs font-semibold capitalize {{ $sc }}">{{ $order['status'] }}</span>
                            </td>
                            <td class="px-5 py-3.5">
                                <a href="{{ route('admin.orders.show', $order['id']) }}" class="text-slate-400 transition hover:text-indigo-600">
                                    <i data-lucide="eye" class="h-4 w-4"></i>
                                </a>
                            </td>
                        </tr>
                        @empty
                        <tr><td colspan="6" class="px-5 py-10 text-center text-sm text-slate-500">No recent orders found.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
        ...
        <article class="rounded-2xl border border-slate-100 bg-white p-5 shadow-sm">
            <div class="mb-5 flex items-center justify-between">
                ...
                <a href="{{ route('admin.products.index') }}" class="text-sm font-semibold text-indigo-600 hover:text-indigo-700 transition">View All</a>
            </div>
            <div class="space-y-3">
                @forelse($topProducts as $product)
                <div class="flex items-center gap-3 rounded-xl border border-slate-100 p-3 transition hover:bg-slate-50">
                    ...
                    <div class="text-right">
                        <p class="text-sm font-bold text-slate-900">৳{{ number_format($product['revenue'],0) }}</p>
    ...
    {{-- Inventory Alerts + Customer Activity --}}
    <section class="grid gap-6 lg:grid-cols-2">
        ...
        <article class="rounded-2xl border border-slate-100 bg-white p-5 shadow-sm">
            ...
            <div class="space-y-3">
                ...
                <div class="flex items-center gap-3 rounded-xl border p-3.5 {{ $cardCls }}">
                    ...
                    <span class="shrink-0 rounded-full px-2.5 py-1 text-xs font-bold {{ $badgeCls }}">{{ $status }}</span>
                </div>
                @empty
                <div class="rounded-xl border border-emerald-100 bg-emerald-50/60 p-6 text-center">
                    <div class="mx-auto flex h-10 w-10 items-center justify-center rounded-xl bg-white text-emerald-600">
        ...
        <article class="rounded-2xl border border-slate-100 bg-white p-5 shadow-sm">
            <div class="mb-5 flex items-center justify-between">
                <div>
                    <h2 class="text-base font-bold text-slate-900">Customer Activity</h2>
            ...
            <div class="space-y-0">
                @php
                    $activities = [
                        ['icon'=>'shopping-bag','bg'=>'bg-emerald-50','color'=>'text-emerald-600','title'=>'New order placed by Ayesha Rahman','desc'=>'Purchased Wireless Headphones for ৳3,200','time'=>'2 minutes ago'],
                        ['icon'=>'user-plus',   'bg'=>'bg-blue-50',   'color'=>'text-blue-600',   'title'=>'18 new customers registered','desc'=>'Most signups came from mobile app campaign','time'=>'28 minutes ago'],
                        ['icon'=>'star',        'bg'=>'bg-violet-50', 'color'=>'text-violet-600', 'title'=>'Product review received','desc'=>'Smart Watch Pro received a 5-star review','time'=>'1 hour ago'],
                        ['icon'=>'rotate-ccw',  'bg'=>'bg-rose-50',   'color'=>'text-rose-600',   'title'=>'Refund request submitted','desc'=>'Order #ORD-7838 is awaiting approval','time'=>'3 hours ago'],
                    ];
                @endphp
                @foreach($activities as $i => $act)
                <div class="relative flex gap-3 {{ !$loop->last ? 'pb-5' : '' }}">
                    @if(!$loop->last)
                        <div class="absolute left-[19px] top-10 bottom-0 w-px bg-slate-100"></div>
                    @endif
                    <div class="grid h-10 w-10 shrink-0 place-items-center rounded-full {{ $act['bg'] }} {{ $act['color'] }}">
                        <i data-lucide="{{ $act['icon'] }}" class="h-4 w-4"></i>
                    </div>
                    <div>
                        <p class="text-sm font-semibold text-slate-900">{{ $act['title'] }}</p>
                        <p class="mt-0.5 text-sm text-slate-500">{{ $act['desc'] }}</p>
                        <p class="mt-1 text-xs text-slate-400">{{ $act['time'] }}</p>
                    </div>
                </div>
                @endforeach
            </div>
