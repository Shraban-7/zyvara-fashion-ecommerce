...
<div class="space-y-6">
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h3 class="text-2xl font-bold text-secondary-800">Edit Static Page</h3>
            <p class="mt-1 text-sm text-secondary-500">Update content, SEO metadata and publication settings.</p>
        </div>
    ...
    <form id="staticPageForm" action="{{ route('admin.static_pages.update', $page->id) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="rounded-2xl border border-secondary-200 bg-white p-6 shadow-sm">
            ...
            <div class="mt-5 grid grid-cols-1 gap-5 md:grid-cols-2">
                ...
                <div class="md:col-span-2">
                    ...
                    <p class="mt-1 text-xs text-secondary-500">Public URL: /pages/{{ old('slug', $page->slug) }}</p>
                </div>
                ...
                <div>
                    ...
                    <input type="number" id="footer_position" name="footer_position" value="{{ old('footer_position', $page->footer_position) }}" min="1"
                        class="w-full rounded-lg border border-secondary-300 px-3 py-2.5 text-sm focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary-200">
                </div>
            ...
            <div class="mt-5 grid grid-cols-1 gap-5">
                ...
                <div>
                    ...
                    <textarea id="meta_description" name="meta_description" rows="4"
                        class="w-full rounded-lg border border-secondary-300 px-3 py-2.5 text-sm focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary-200"
                        placeholder="Short description for search results">{{ old('meta_description', $page->meta_description) }}</textarea>
                </div>
