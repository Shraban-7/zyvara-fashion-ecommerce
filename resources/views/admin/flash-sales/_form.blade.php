...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6" x-data="flashSaleForm(@js($selected))">
    ...
    <div class="lg:col-span-1 space-y-5">
        <div class="bg-white p-5 rounded-xl border border-secondary-200 shadow-sm space-y-4">
            <h3 class="text-sm font-bold text-secondary-800 uppercase tracking-wide">Details</h3>
            ...
            <div>
                <label class="block text-sm font-medium text-secondary-700 mb-2">Starts At *</label>
            ...
            <div>
                <label class="block text-sm font-medium text-secondary-700 mb-2">Ends At *</label>
    ...
    <div class="lg:col-span-2 space-y-5">
        <div class="bg-white p-5 rounded-xl border border-secondary-200 shadow-sm">
            <h3 class="text-sm font-bold text-secondary-800 uppercase tracking-wide mb-4">Products</h3>
            ...
            <div class="relative mb-4">
                ...
                <div x-show="results.length" x-cloak
                    class="absolute z-20 mt-1 w-full bg-white border border-secondary-200 rounded-lg shadow-lg max-h-72 overflow-y-auto">
                    <template x-for="p in results" :key="p.id">
                        <button type="button" @click="addProduct(p)"
                            class="flex items-center gap-3 w-full px-4 py-2.5 text-left hover:bg-secondary-50 border-b border-gray-100 last:border-0">
                            <img :src="p.image || 'https://placehold.co/40x40?text=—'" class="h-9 w-9 rounded object-cover bg-secondary-100">
                            <div class="min-w-0 flex-1">
                                <div class="text-sm font-medium text-secondary-800 truncate" x-text="p.name"></div>
                                <div class="text-xs text-secondary-400" x-text="'SKU: ' + (p.sku || '—') + ' · ৳' + p.price"></div>
                            </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-sm text-left text-secondary-600">
                    <thead class="text-xs text-secondary-500 uppercase bg-secondary-50 border-b">
                        <tr>
                    ...
                    <tbody>
                            ...
                            <tr class="border-b border-gray-100">
                                <td class="px-3 py-2">
                                    <div class="flex items-center gap-2">
                                        <img :src="p.image || 'https://placehold.co/36x36?text=—'" class="h-8 w-8 rounded object-cover bg-secondary-100">
                                        <span class="text-sm text-secondary-800" x-text="p.name"></span>
                                    </div>
                                    <input type="hidden" name="products[]" :value="p.id">
                                </td>
                                <td class="px-3 py-2 text-secondary-500" x-text="'৳' + p.price"></td>
                                <td class="px-3 py-2">
                                    <input type="number" step="0.01" min="0"
                                        :name="'sale_prices[' + p.id + ']'"
                                        x-model="p.sale_price"
                                        placeholder="Leave blank = regular"
                                        class="w-full px-2 py-1.5 rounded border border-secondary-300 text-sm focus:border-primary focus:ring-1 focus:ring-primary">
                                </td>
                                <td class="px-3 py-2 text-right">
                                    <button type="button" @click="removeProduct(idx)" class="text-danger hover:text-danger">
                        ...
                        <tr x-show="!selected.length">
                            <td colspan="4" class="px-3 py-8 text-center text-secondary-400 italic">No products added yet.</td>
                        </tr>
                    </tbody>
                </table>
            </div>
