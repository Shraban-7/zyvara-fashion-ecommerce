...
<div >
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
            ...
            <p class="text-sm text-gray-500">Track all user actions across the system</p>
        </div>
        <span class="inline-flex items-center px-3 py-1 rounded-md bg-indigo-50 text-indigo-700 border border-indigo-200 text-sm font-medium">
            <i class="fas fa-list-alt mr-2"></i> {{ $activities->total() }} Records
        </span>
    </div>

    {{-- Filters --}}
    <form method="GET" class="bg-white border border-gray-200 rounded-xl p-4 mb-6 shadow-sm">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-3">

            <div class="lg:col-span-2">
                <input type="text" name="search" value="{{ request('search') }}"
                    placeholder="Search user, action, model, IP..."
                    class="w-full h-10 px-4 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
            </div>

            <div>
                <select name="action" class="w-full h-10 px-3 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500">
            ...
            <div>
                <select name="model_type" class="w-full h-10 px-3 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500">
            ...
            <div class="flex gap-2">
                <button type="submit"
                    class="flex-1 h-10 px-4 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition text-sm font-medium flex items-center justify-center">
                    <i class="fas fa-filter mr-1"></i> Filter
                </button>
                <a href="{{ route('admin.activity-logs.index') }}"
                    class="h-10 px-3 border border-gray-300 text-gray-600 rounded-lg hover:bg-gray-50 transition flex items-center justify-center">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-3 mt-3">
            <div class="flex items-center gap-2">
                <span class="text-sm text-gray-500 whitespace-nowrap">From:</span>
                <input type="date" name="date_from" value="{{ request('date_from') }}"
                    class="flex-1 h-10 px-3 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500">
            </div>
            <div class="flex items-center gap-2">
            ...
            <table class="min-w-full divide-y divide-gray-200 text-sm">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-center font-semibold text-gray-600">Details</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50 transition">
                        ...
                        <td class="px-4 py-3">
                            <div class="flex items-center gap-2">
                                <div class="h-7 w-7 rounded-full bg-indigo-100 text-indigo-600 flex items-center justify-center font-bold text-xs flex-shrink-0">
                                    {{ strtoupper(substr($log->user->name ?? '?', 0, 1)) }}
                                </div>
                                <span class="font-medium text-gray-800">{{ $log->user->name ?? '<em class="text-gray-400">Deleted</em>' }}</span>
                            </div>
                        </td>
                        ...
                        <td class="px-4 py-3">
                            ...
                            $color = $actionColors[$log->action] ?? 'bg-purple-100 text-purple-700';
                            @endphp
                        ...
                        <td class="px-4 py-3">
                            @if($log->model_type)
                            <span class="text-gray-700 font-medium">{{ class_basename($log->model_type) }}</span>
                            @if($log->model_id)
                        ...
                        <td class="px-4 py-3 text-center">
                            <button onclick="openOffcanvas(this)"
                                ...
                                class="inline-flex items-center px-2.5 py-1 rounded-lg bg-gray-100 text-gray-600 hover:bg-indigo-100 hover:text-indigo-700 transition text-xs font-medium">
...
<div id="offcanvas"
    ...
    <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100 shrink-0">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h3 class="text-base font-bold text-gray-800">Log Details</h3>
                <p id="oc-id" class="text-xs text-gray-400">#—</p>
            </div>
        </div>
        <button onclick="closeOffcanvas()"
            class="h-8 w-8 rounded-lg text-gray-400 hover:text-gray-700 hover:bg-gray-100 transition flex items-center justify-center">
            <i class="fas fa-times"></i>
        </button>
    </div>
    ...
    <div class="flex-1 overflow-y-auto px-6 py-5 space-y-6">
        ...
        <div class="grid grid-cols-2 gap-3">
            <div class="bg-gray-50 rounded-xl p-3">
                <p class="text-[10px] font-semibold text-gray-400 uppercase tracking-wider mb-1">User</p>
                <p id="oc-user" class="text-sm font-semibold text-gray-800">—</p>
            </div>
            <div class="bg-gray-50 rounded-xl p-3">
            ...
            <div class="bg-gray-50 rounded-xl p-3">
                <p class="text-[10px] font-semibold text-gray-400 uppercase tracking-wider mb-1">Model</p>
                <p id="oc-model" class="text-sm font-semibold text-gray-800">—</p>
            </div>
            <div class="bg-gray-50 rounded-xl p-3">
                <p class="text-[10px] font-semibold text-gray-400 uppercase tracking-wider mb-1">Date</p>
                <p id="oc-date" class="text-sm font-semibold text-gray-800">—</p>
            </div>
        </div>
        ...
        <div id="oc-desc-wrap" class="hidden">
            ...
            <p id="oc-desc" class="text-sm text-gray-700 bg-gray-50 rounded-xl px-4 py-3 leading-relaxed"></p>
        </div>

        {{-- Network --}}
        <div>
            <p class="text-[10px] font-semibold text-gray-400 uppercase tracking-wider mb-2">Network</p>
            <div class="bg-gray-50 rounded-xl divide-y divide-gray-100">
                <div class="flex items-start gap-3 px-4 py-3">
                    <i class="fas fa-network-wired text-gray-400 mt-0.5 w-4 text-center"></i>
                    <div class="min-w-0">
                        <p class="text-[10px] text-gray-400 font-medium uppercase">IP Address</p>
                        <p id="oc-ip" class="text-sm text-gray-700 font-mono">—</p>
                    </div>
                </div>
                <div id="oc-address-row" class="flex items-start gap-3 px-4 py-3 hidden">
                    ...
                    <div class="min-w-0">
                        <p class="text-[10px] text-gray-400 font-medium uppercase">Address</p>
                        <p id="oc-address" class="text-sm text-gray-700 break-words"></p>
                    </div>
                </div>
                <div id="oc-ua-row" class="flex items-start gap-3 px-4 py-3 hidden">
                    <i class="fas fa-laptop text-gray-400 mt-0.5 w-4 text-center"></i>
                    <div class="min-w-0">
                        <p class="text-[10px] text-gray-400 font-medium uppercase">User Agent</p>
                        <p id="oc-ua" class="text-xs text-gray-500 break-all leading-relaxed"></p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div id="oc-old-wrap" class="hidden">
            <p class="text-[10px] font-semibold text-gray-400 uppercase tracking-wider mb-2">
                <i class="fas fa-minus-circle text-red-400 mr-1"></i> Old Values
            </p>
            <div class="rounded-xl border border-red-100 overflow-hidden">
                <table class="min-w-full text-sm">
                    <tbody id="oc-old-body" class="divide-y divide-red-50"></tbody>
                </table>
            </div>
        </div>
        ...
        <div id="oc-new-wrap" class="hidden">
            <p class="text-[10px] font-semibold text-gray-400 uppercase tracking-wider mb-2">
                <i class="fas fa-plus-circle text-green-400 mr-1"></i> New Values
            </p>
            <div class="rounded-xl border border-green-100 overflow-hidden">
                <table class="min-w-full text-sm">
                    <tbody id="oc-new-body" class="divide-y divide-green-50"></tbody>
                </table>
            </div>
        </div>
...
<script>
    ...
    function openOffcanvas(btn) {
        ...
        // Action badge
        const color = actionColors[log.action] ?? 'bg-purple-100 text-purple-700';
        ...
        // Old Values
        fillValuesTable('oc-old-wrap', 'oc-old-body', log.old_values, 'bg-red-50', 'text-red-700');

        // New Values
        fillValuesTable('oc-new-wrap', 'oc-new-body', log.new_values, 'bg-green-50', 'text-green-700');
    ...
    function fillValuesTable(wrapId, bodyId, values, rowBg, textColor) {
        ...
        if (values && Object.keys(values).length) {
            Object.entries(values).forEach(([key, val]) => {
                const display = (val === null || val === undefined) ? '<em class="text-gray-400">null</em>' :
                    (typeof val === 'object' ? `<code class="text-xs">${JSON.stringify(val)}</code>` : val);
                body.insertAdjacentHTML('beforeend',
                    `<tr class="${rowBg}">
                        <td class="px-4 py-2 font-medium text-gray-600 w-1/3 align-top">${key}</td>
