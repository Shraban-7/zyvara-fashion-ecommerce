    ...
    <div class="space-y-6">
        ...
        <div class="mb-6">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-2xl md:text-3xl font-bold text-primary mb-2">Manage Stock</h1>
        ...
        @if($product->variants->isEmpty())
            <div class="bg-white rounded-xl border border-secondary-200 shadow-sm p-4 max-w-2xl">
                <div class="flex items-center justify-between border-b border-gray-100 pb-3 mb-4">
                    <div>
                        <h2 class="text-sm font-bold text-primary">Base Product Stock</h2>
                        <p class="text-xs text-secondary-400 mt-0.5">SKU: {{ $product->sku ?? 'N/A' }}</p>
                    </div>
                    <div class="text-right">
                        <span class="text-[11px] font-medium text-secondary-500 block uppercase tracking-wider">Current</span>
                ...
                <form class="stock-form" data-product-id="{{ $product->id }}">
                    ...
                    <div class="grid grid-cols-12 gap-3 items-end">
                        ...
                        <div class="col-span-3">
                            <label class="block text-[11px] font-semibold text-secondary-600 mb-1">Action</label>
                            <div class="flex p-0.5 bg-secondary-100 rounded-lg">
                                <label class="flex-1 cursor-pointer text-center">
                                    <input type="radio" name="action_type" value="add" class="sr-only peer" checked>
                                    <span
                                        class="block py-1 text-xs font-medium text-secondary-600 rounded-md peer-checked:bg-white peer-checked:text-success peer-checked:shadow-sm transition">Add</span>
                                </label>
                                <label class="flex-1 cursor-pointer text-center">
                                    <input type="radio" name="action_type" value="remove" class="sr-only peer">
                                    <span
                                        class="block py-1 text-xs font-medium text-secondary-600 rounded-md peer-checked:bg-white peer-checked:text-danger peer-checked:shadow-sm transition">Remove</span>
                                </label>
                            </div>
                        </div>
                        ...
                        <!-- Quantity -->
                        <div class="col-span-3">
                            <label class="block text-[11px] font-semibold text-secondary-600 mb-1">Qty</label>
                            <input type="number" name="quantity" min="1" required placeholder="Qty"
                                class="quantity-input w-full px-2 py-1.5 text-xs border border-secondary-300 rounded-lg focus:border-primary focus:ring-1 focus:ring-primary outline-none">
                        </div>

                        <!-- Note -->
                        <div class="col-span-3">
                            <label class="block text-[11px] font-semibold text-secondary-600 mb-1">Note <span
                                    class="text-secondary-400 font-normal">(opt)</span></label>
                            <input type="text" name="note" placeholder="Reason..."
                                class="w-full px-2.5 py-1.5 text-xs border border-secondary-300 rounded-lg focus:border-primary focus:ring-1 focus:ring-primary outline-none">
                        </div>

                        <!-- Submit Button -->
                        <div class="col-span-3">
                            <button type="submit" disabled
                                class="submit-btn w-full inline-flex items-center justify-center gap-1 px-3 py-2 bg-success text-white rounded-lg hover:bg-success-700 active:bg-success-800 transition shadow-sm text-xs font-semibold">
        ...
        @if($product->variants->isNotEmpty())
            <div class="bg-white rounded-xl border border-secondary-200 shadow-sm ">
                {{-- Prevent horizontal scroll --}}
                <div class="divide-y divide-gray-100">
                    @foreach($product->variants as $variant)
                        <div class="p-4 hover:bg-secondary-50/50 transition">
                            <form class="stock-form" data-variant-id="{{ $variant->id }}">
                                ...
                                <div class="grid grid-cols-1 lg:grid-cols-12 gap-3 items-center">
                                    ...
                                    <div class="lg:col-span-3">
                                        <h3 class="text-xs font-bold text-primary truncate" title="{{ $variant->variant_name }}">
                                            {{ $variant->variant_name }}
                                        </h3>

                                        <p class="text-[10px] text-secondary-400 truncate mt-0.5">
                                            SKU: {{ $variant->sku ?? 'N/A' }}
                                        </p>
                                    </div>
                                    ...
                                    <div class="lg:col-span-1 text-center lg:border-r border-gray-100 lg:pr-2">
                                        <span class="text-[10px] text-secondary-400 block uppercase">Current Stock</span>

                                        <span class="text-sm font-black text-secondary-800 stock-display"
                                    ...
                                    <div class="lg:col-span-2">
                                        <div class="grid grid-cols-2 gap-1 p-0.5 bg-secondary-100 rounded-lg">
                                            <label class="cursor-pointer">
                                                <input type="radio" name="action_type" value="add" class="hidden radio-input"
                                                    checked>
                                                <span
                                                    class="block py-1 text-center text-[11px] font-semibold text-secondary-500 rounded-md radio-label transition">
                                    ...
                                    <div class="lg:col-span-3">
                                        <input type="text" name="note" placeholder="Note (optional)"
                                            class="w-full px-2 py-1.5 text-xs border border-secondary-300 rounded-lg focus:border-primary focus:ring-1 focus:ring-primary outline-none">
                                    </div>
                    ...
                    function updateButtonState() {
                        ...
                        if (actionType === 'add') {
                            ...
                            submitBtn.classList.remove(
                                'bg-danger', 'hover:bg-danger-700', 'active:bg-danger-800'
                            );

                            submitBtn.classList.add(
                                'bg-success', 'hover:bg-success-700', 'active:bg-success-800'
                            );
